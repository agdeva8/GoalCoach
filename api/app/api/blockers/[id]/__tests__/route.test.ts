...
const BLOCK_ID = 'block_a1b2c3d4e5f6'
...
function putReq(body: object): NextRequest {
  ...
  return new NextRequest(`http://localhost/api/blockers/${BLOCK_ID}`, {
    method: 'PUT',
    headers,
...
describe('PUT /api/blockers/[id]', () => {
  ...
  it('returns 401 without auth', async () => {
    ...
    const req = new NextRequest(`http://localhost/api/blockers/${BLOCK_ID}`, {
      method: 'PUT',
      headers,
      body: JSON.stringify({ title: 'Travel', start_date: '2026-12-20' }),
    })
    const res = await PUT(req, { params: Promise.resolve({ id: BLOCK_ID }) })
  ...
  it('returns 404 when not owned', async () => {
    selectResultHolder.current = []
    const res = await PUT(
      putReq({ title: 'Travel', start_date: '2026-12-20' }),
      { params: Promise.resolve({ id: BLOCK_ID }) },
    )
    expect(res.status).toBe(404)
  })

  it('returns 400 on malformed start_date', async () => {
    selectResultHolder.current = [{ id: BLOCK_ID, title: 'Old' }]
    const res = await PUT(
      putReq({ title: 'Travel', start_date: 'soon' }),
      { params: Promise.resolve({ id: BLOCK_ID }) },
    )
    expect(res.status).toBe(400)
  })

  it('updates blocker + writes audit', async () => {
    selectResultHolder.current = [{ id: BLOCK_ID, title: 'Old' }]
    const res = await PUT(
      putReq({
        title: 'Travel (extended)',
    ...
    expect(setArg.title).toBe('Travel (extended)')
  })
})
...
describe('DELETE /api/blockers/[id]', () => {
  ...
  it('returns 401 without auth', async () => {
    ...
    const res = await DELETE(req, { params: Promise.resolve({ id: BLOCK_ID }) })
    expect(res.status).toBe(401)
  })
  ...
  it('deletes blocker + writes audit', async () => {
    selectResultHolder.current = [{ id: BLOCK_ID, title: 'Old' }]
    const res = await DELETE(deleteReq(), {
      params: Promise.resolve({ id: BLOCK_ID }),
    })
